...
def load_data(sheet_name, columns=None):
    try:
        # ttl=0 untuk memastikan data sentiasa dikemaskini
...
def generate_sijil(name, perc, gb):
    ...
    pdf.multi_cell(0, 10, f"Atas pencapaian kehadiran asrama sebanyak {perc:.1f}%", align='C')
    pdf.ln(15)
    pdf.cell(0, 10, f"({gb.upper()})", ln=True, align='C')
...
if check_password():
    ...
    # --- TAB 2: MAKLUMAT ASRAMA (CARIAN & 20 BARIS) ---
    with tabs[1]:
        st.subheader("🏠 Pengurusan Penghuni Asrama")
        if not df_asrama.empty:
            # CARIAN LUARAN: Selesai masalah "Rachel"
            q = st.text_input("🔍 Cari Nama atau No. KP (Contoh: rachel):", placeholder="Taip di sini untuk mencari...")
            if q:
            ...
            # PAGINATION: Selesai masalah paparan 9 orang
            limit = 20
    ...
    with tabs[2]:
        ...
        sel_date = st.date_input("Pilih Tarikh Rekod:", value=date.today())
        ...
        if not df_asrama.empty:
            with st.form("att_form"):
                ...
                for idx, row in df_asrama.iterrows():
                    ...
                    c_n.write(row['NAMA'])
                    # Default kepada data lama jika sedang backdate
        ...
        if not df_kehadiran.empty:
            ...
            if not layak.empty:
                ...
                if st.button("📜 Jana Sijil PDF"):
                    pdf_s = generate_sijil(sel_cert, layak.loc[sel_cert, 'Peratus (%)'], gb_name)
            ...
            st.download_button("📥 Eksport Laporan Kehadiran (Excel)", to_excel(df_kehadiran), "Laporan_Kehadiran.xlsx")
    ...
    with tabs[5]:
        ...
        with st.form("set_form"):
            ...
            if st.form_submit_button("💾 Simpan Tetapan"):
                ...
                st.success("Tetapan telah dikemaskini!")
